...
import { queries, mutations } from "~/lib/api/organizer";
...
interface UpdateInterviewRoundDTO extends Partial<InterviewRoundDTO> {
  id: string;
}
...
function Component() {
  ...
  >({
    mutationFn: (roundData) =>
      mutations.createInterviewRound({ data: roundData }),
    onSuccess: () => {
  ...
  >({
    mutationFn: (updateData) =>
      mutations.updateInterviewRound({ data: updateData }),
  ...
  const setIntervieweeListMutation = useMutation<unknown, any, EmailListData>({
    ...
    onError: (error: any) =>
      toast.error(
        `Set Interviewee List failed: ${error.message || "Unknown error"}`
      ),
  });
  ...
  const setInterviewerListMutation = useMutation<unknown, any, EmailListData>({
    ...
    onError: (error: any) =>
      toast.error(
        `Set Interviewer List failed: ${error.message || "Unknown error"}`
      ),
  });
  ...
  const handleFetchRoundPreview = async () => {
    ...
    try {
      ...
      toast.error(
        `Fetch Round Preview failed: ${error.message || "Unknown error"}`
      );
  ...
  const updateRoundData: UpdateInterviewRoundDTO = {
    id: "ivro_01JT9632V1YKZXE3YFPG0QCV8P", // Replace with valid ID
  ...
  return (
    <main className="container mx-auto p-4 space-y-4">
      ...
      <div className="flex gap-3 flex-wrap">
        ...
        >
          {setIntervieweeListMutation.isLoading
            ? "Updating..."
            : "Set Interviewee List"}
        </Button>
        ...
        >
          {setInterviewerListMutation.isLoading
            ? "Updating..."
            : "Set Interviewer List"}
        </Button>

        <Button
          onClick={() =>
            scheduleRoundMutation.mutate({ roundId: updateRoundData.id })
          }
          disabled={scheduleRoundMutation.isLoading}
        >
          {scheduleRoundMutation.isLoading
            ? "Scheduling..."
        ...
        <Button
          onClick={() =>
            openRoundMutation.mutate({ roundId: updateRoundData.id })
          }
        ...
        <Button
          onClick={() =>
            endRoundMutation.mutate({ roundId: updateRoundData.id })
          }
        ...
        <Button onClick={handleFetchUserRounds}>
          Fetch My Interview Rounds
        </Button>

        <Button onClick={handleFetchRoundPreview}>
          Fetch Interview Round Preview
        </Button>
      </div>
      ...
      {createRoundMutation.isError && (
        <p className="text-red-500">
          Create Error: {createRoundMutation.error?.message}
        </p>
      )}
      {updateRoundMutation.isError && (
        <p className="text-red-500">
          Update Error: {updateRoundMutation.error?.message}
        </p>
      )}
      ...
      {openRoundMutation.isError && (
        <p className="text-red-500">
          Open Error: {openRoundMutation.error?.message}
        </p>
      )}
      {endRoundMutation.isError && (
        <p className="text-red-500">
          End Error: {endRoundMutation.error?.message}
        </p>
      )}
      ...
      <div className="inline-flex items-center justify-between gap-3 mt-8">
        ...
        <pre>Suspense Echo: {JSON.stringify(echoQuery.data, null, 2)}</pre>
      </div>
