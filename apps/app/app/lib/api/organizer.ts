...
const getInterviewRoundOrganizerPreviewHandler = createServerFn({
	...
	.handler<InterviewRoundViewData>(async ({ data, context }) => {
		...
		try {
			...
			const interviewSlotsAssignedDrizzle = await db.query.interview_slot.findMany({
...
const getHostedInterviewRoundsHandler = createServerFn({
	...
	.handler<InterviewRoundData[]>(async ({ context }) => {
		...
		if (!context.session?.user) {
			console.error("Middleware failed to attach user or user ID in GET list handler.");
			throw new Error("Unauthorized: Authentication context missing.");
		}
		...
		try {
			...
			throw new Error("Failed to retrieve interview rounds due to a server error.");
		}
	});
...
const createInterviewRoundHandler = createServerFn({ method: "POST" })
	...
	.handler(async ({ data, context }: any) => {
		...
		if (!userId) {
			console.error("Middleware failed to attach user or user ID in POST handler.");
			throw new Error("Unauthorized: Authentication context missing.");
		}

		console.log(`Attempting to create an interview round for organizer ID: ${userId}`);
		...
		try {
			...
			console.log(`Successfully created an interview round for organizer ID: ${userId}`);
...
const updateInteriviewRound = createServerFn({ method: "POST" })
	...
	.handler(async ({ data, context }: any) => {
		...
		console.log(`Attempting to update an interview round for organizer ID: ${userId}`);

		const interviewRoundDTO = data.interview_round;

		const updateData: Partial<typeof schema.interview_round.$inferInsert> = {};

		if (interviewRoundDTO.title !== undefined && interviewRoundDTO.title !== null) {
		...
		if (
			...
			if (!isNaN(startDate.getTime())) {
				...
				console.warn("Received invalid start_date format:", interviewRoundDTO.start_date);
				throw new Error("Invalid start date format.");
			}
		}
		if (interviewRoundDTO.end_date !== undefined && interviewRoundDTO.end_date !== null) {
			const endDate = new Date(interviewRoundDTO.end_date);
			if (!isNaN(endDate.getTime())) {
				updateData.end_date = endDate;
			} else {
				console.warn("Received invalid end_date format:", interviewRoundDTO.end_date);
...
const scheduleInterviewRound = createServerFn({ method: "POST" })
	...
	.handler(async ({ context, data }) => {
		...
		try {
			...
			throw new Error("Error updateing the status of an interview round in `started`");
		}
	});
...
const stopInterviewRound = createServerFn({ method: "POST" })
	...
	.handler(async ({ context, data }) => {
		...
		try {
			...
			throw new Error("Error updateing the status of an interview round in `ended`");
		}
	});
...
export const queries = {
	...
	interviewRoundView: (roundId: string) =>
		queryOptions<InterviewRoundViewData>({
			...
			queryFn: () => getInterviewRoundOrganizerPreviewHandler({ data: { roundId } }),
		}),
};
