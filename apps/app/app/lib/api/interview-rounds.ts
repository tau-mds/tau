...
export const queries = {
	all: () =>
		queryOptions({
			...
			queryFn: (opts) =>
				all({ signal: opts.signal }) as Promise<Array<schema.interview_round>>,
		}),
	id: (id: ids.interview_round) =>
		queryOptions({
			queryKey: ["interviewRounds", id],
			queryFn: (opts) =>
				round({ data: { id }, signal: opts.signal }) as Promise<schema.interview_round>,
		}),
...
const all = createServerFn({ method: "GET" })
