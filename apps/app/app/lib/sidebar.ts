...
function useSetMutation(
  updater: (data: { data: number | undefined }) => number,
) {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (ctx: { data: number }) => set({ data: updater(ctx) }),
    onMutate: async (ctx) => {
      await queryClient.cancelQueries(queries.get());
      queryClient.setQueryData(queries.get().queryKey, updater(ctx));
    },
